...
#admin-products
  ...
  v-row
    v-col(cols="12")
      v-btn.mt-5(color="success" @click="openDialog(-1)") 新增商品
    v-col(cols="12")
      v-table
        thead
          tr
        ...
        tbody
          tr(v-for="(product, idx) in products" :key="product._id")
            ...
            td {{ product.name }}
            td
              v-btn(color="primary" icon="mdi-pencil" @click="openDialog(idx)")
  v-dialog(v-model="form.dialog" persistent)
    v-form.form(v-model="form.valid" @submit.prevent="submit")
      v-card
        v-card-title
          h1.text-center.mt-5.mb-5 {{ form._id.length > 0 ? '編輯商品' : '新增商品' }}
        v-card-text
          v-row
            v-col(cols="12")
